...
function rectSquare(a: number, b: number): number {
  return a * b;
}
...
function circleCylinder(r: number, h: number): number {
  return 2 * Math.PI * r * h + 2 * circleSquare(r);
}
...
function writeList(listItemText: string): void {
  document.write('<ul>');
...
function writeList2(listItemText: string, listLength: number): void {
  document.write('<ul>');
...
function writeArrayOfPrimitives(arr: (string | number | boolean)[]): void {
  document.write('<ul>');
...
function writeArrayOfObjects(
  arr: { id: number; name: string; age: number }[]
): void {
  for (const item of arr) {
    document.write('<div>');
    document.write(
...
function findMinArrayItem(arr: number[]): number {
  let min = arr[0]!;
...
function sumArrayItems(arr: number[]): number {
  let sum = arr[0]!;
...
function swapArrayItems(arr: any[], index1: number, index2: number): any[] {
  let tmp = arr[index1];
