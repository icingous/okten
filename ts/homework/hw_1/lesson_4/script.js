...
function writeList(listItemText) {
    document.write('<ul>');
...
function writeList2(listItemText, listLength) {
    document.write('<ul>');
...
function writeArrayOfPrimitives(arr) {
    document.write('<ul>');
    for (const item of arr) {
...
function writeArrayOfObjects(arr) {
    for (const item of arr) {
        document.write('<div>');
        document.write(`<span>${item.id}</span> <span>${item.name}</span> <span>${item.age}</span>`);
        document.write('</div>');
    }
}
writeArrayOfObjects([
