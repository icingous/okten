...
// створити пустий масив, наповнити його 10 об’єктами new User(….)
{
  ...
  for (let i = 0; i < 10; ++i) {
    ...
    users[i] = new User(
      ...
      `Name${id}`,
      `Surname${id}`,
      `name${id}.surname${id}@google.com`,
      '+38067' + String(i).repeat(7)
    );
  }
  ...
  type Driver = { name: string; licence: string; owner: boolean };
  ...
  class Car {
    ...
    increaseMaxSpeed(increment: number) {
      if (increment > 0) {
  ...
  const trueCinderella = cinderellas.find(
    (item) => item.footSize === prince.slipper.size
  );
  ...
  const cindies = cinderellas.myFilter(
    (c, i, a) => c.footSize > 30 && c.footSize < 35
  );
