...
const sortNums = (array: number[], direction: string): number[] => {
  ...
  const compareDesc = (a: number, b: number): number => b - a;
...
const dockerArr = coursesArray.filter(({ modules }) =>
  modules.includes('docker')
);
...
const getIsValueGT9 = (value: string): boolean =>
...
const clubs = cards.filter(
  (item) => item.cardSuit === 'clubs' && getIsValueGT9(item.value)
);
